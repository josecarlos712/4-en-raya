...
to mouse-player ;hay que activar el boton mouse-manager para que detecte los clicks
  ifelse mouse-down? [
    if parar? [stop]
    if not mouse-clicked? [ ;meter aqui dentro las instrucciones que se ejecutan al hacer click en el tablero
    if ((select-patch) != tamTab) [set estado add-piece estado select-patch]  ;añade una tortuga en la parcela seleccionada *si no es el fin de la columna
