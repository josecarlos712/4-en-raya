...
to setup
  ...
  set estado (list new-matrix tamTab 1) ;crea el estado inicial del tablero
  set parar? false
end
...
to-report add-piece [s x] ;modifica el estado s añadiendo una tortuga en la posicion seleccionada
  ...
  ;en vez de length y filter se podria tmbn count directamente
  let m MCTS:get-content s
  ifelse (item x offset-list) < tamTab [matrix:set m x (item x offset-list) (last s mod 3)] [print "Movimiento ilegal" report MCTS:create-state m (last s - 1)]
